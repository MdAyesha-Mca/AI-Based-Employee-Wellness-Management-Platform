...
st.header("6. Complete Pipeline Integration Testing (Task 5)")

st.write("Verify that data flows correctly across all modules: Ingestion -> Preprocessing -> Sentiment -> Report.")

integration_samples = [
    "I love how smooth this experience has been!",
    "This was a huge disappointment, nothing worked.",
    "The package arrived on Thursday as scheduled.",
    "",
    "   ",
    "Amazing!!! Best decision ever made",
...
if st.button("Run Full Pipeline Test"):
    ...
    for i, raw_input in enumerate(integration_samples, start=1):
        row = {"Sample #": i, "Raw Input": repr(raw_input)}

        is_valid = validate_text(raw_input)
        row["Ingestion Valid?"] = "Yes" if is_valid else "No"

        if not is_valid:
            row["Preprocessing Output"] = "Skipped (invalid input)"
            row["Sentiment Label"] = "Skipped (invalid input)"
            row["Compound Score"] = "N/A"
            row["Status"] = "Handled correctly"
            pipeline_rows.append(row)
            continue

        try:
            prep_result = preprocess(raw_input)
            if "error" in prep_result:
                row["Preprocessing Output"] = f"Error: {prep_result['error']}"
                row["Sentiment Label"] = "Skipped"
                row["Compound Score"] = "N/A"
                row["Status"] = "Handled correctly"
                pipeline_rows.append(row)
                continue
            processed_text = prep_result["final_processed_text"]
            row["Preprocessing Output"] = processed_text
        except Exception as e:
            row["Preprocessing Output"] = f"CRASH: {e}"
            row["Sentiment Label"] = "N/A"
            row["Compound Score"] = "N/A"
            row["Status"] = "INTEGRATION ERROR"
            errors_found.append(f"Sample {i}: Preprocessing crashed - {e}")
            pipeline_rows.append(row)
            continue

        try:
            sentiment_result = analyze_sentiment(raw_input)
            if "error" in sentiment_result:
                row["Sentiment Label"] = f"Error: {sentiment_result['error']}"
                row["Compound Score"] = "N/A"
                row["Status"] = "Handled correctly"
                pipeline_rows.append(row)
                continue
            row["Sentiment Label"] = sentiment_result["label"]
            row["Compound Score"] = round(sentiment_result["compound"], 3)
        except Exception as e:
            row["Sentiment Label"] = f"CRASH: {e}"
            row["Compound Score"] = "N/A"
            row["Status"] = "INTEGRATION ERROR"
            errors_found.append(f"Sample {i}: Sentiment analysis crashed - {e}")
            pipeline_rows.append(row)
            continue

        row["Status"] = "Pipeline OK"
        pipeline_rows.append(row)

    pipeline_df = pd.DataFrame(pipeline_rows)

    if errors_found:
        st.error(f"{len(errors_found)} integration error(s) found!")
        for err in errors_found:
            st.write("-", err)
    else:
        st.success("All samples passed through the pipeline with no integration errors.")

    st.subheader("Pipeline Trace (Ingestion -> Preprocessing -> Sentiment)")
    st.dataframe(pipeline_df, use_container_width=True)

    passed = len(pipeline_df[pipeline_df["Status"] != "INTEGRATION ERROR"])
    total = len(pipeline_df)
    col1, col2 = st.columns(2)
    col1.metric("Samples Tested", total)
    col2.metric("Handled Without Crashing", f"{passed}/{total}")